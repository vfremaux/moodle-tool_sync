...
class users_sync_manager extends sync_manager {
    ...
    public function form_elements(&$frm) {
        ...
        $key = 'tool_sync/users_deletefilelocation';
        $label = get_string('deleteusersfile', 'tool_sync');
        $frm->addElement('text', $key, $label);
        $frm->setType('tool_sync/users_deletefilelocation', PARAM_TEXT);
        $frm->addHelpButton($key, 'localfile', 'tool_sync');

        $barr = array();

        $cronurl = new \moodle_url('/admin/tool/sync/users/execcron.php', array('action' => SYNC_USER_ADD));
        $params = array('onclick' => 'document.location.href= \''.$cronurl.'\'');
        $barr[] = $frm->createElement('button', 'manualusers', get_string('manualuserrun', 'tool_sync'), $params);

        $cronurl = new \moodle_url('/admin/tool/sync/users/execcron.php', array('action' => SYNC_USER_DELETE));
        $params = array('onclick' => 'document.location.href= \''.$cronurl.'\'');
        $barr[] = $frm->createElement('button', 'manualdeleteusers', get_string('manualdeleteuserrun', 'tool_sync'), $params);

        $cronurl = new \moodle_url('/admin/tool/sync/users/execcron.php', array('action' => SYNC_USER_SUSPEND));
        $params = array('onclick' => 'document.location.href= \''.$cronurl.'\'');
        $barr[] = $frm->createElement('button', 'manualsuspendusers', get_string('manualsuspenduserrun', 'tool_sync'), $params);

        $cronurl = new \moodle_url('/admin/tool/sync/users/execcron.php', array('action' => SYNC_USER_RESTORE));
        $params = array('onclick' => 'document.location.href= \''.$cronurl.'\'');
        $barr[] = $frm->createElement('button', 'manualrestoreusers', get_string('manualrestoreuserrun', 'tool_sync'), $params);

        $frm->addGroup($barr, 'manualusers', get_string('manualhandling', 'tool_sync'), array('&nbsp;&nbsp;'), false);
    }
     ...
     */
    public function cron($syncconfig, $action = SYNC_USER_ADD) {
        ...
        if ($this->execute & SYNC_USER_ADD) {

            $this->report(get_string('usersstarting', 'tool_sync'));
                    ...
                    if (!empty($addcourses)) {
                        foreach ($addcourses as $c) {
                            ...
                            if (empty($c->wwwroot)) {
                                ...
                                if (!empty($c->enrol)) {
                                    // Explicit enrol method. So its an enrolment.
                                ...
                                if (!empty($c->role)) {
                                    ...
                                    if (!empty($c->enrol)) {
                                        ...
                                        try {
                                            ...
                                            $this->report(get_string('errorenrol', 'tool_sync', $e).' '.$exc->getMessage());
                                        }
                                    } else {
                                    ...
                                    // We default to student role if not given in the file.
                                    if (!empty($c->enrol)) {
        ...
        if ($this->execute & SYNC_USER_DELETE) {

            $this->report(get_string('startingdeletingusers', 'tool_sync'));
            ...
            if (empty($this->manualfilerec)) {
                $filerec = $this->get_input_file(@$syncconfig->users_deletefilelocation, 'userdelete.csv');
            } else {
                $filerec = $this->manualfilerec;
            }
            ...
            while (!feof($filereader)) {
                ...
                $identity = $record[$syncconfig->users_primaryidentity];

                // ensure user exists with this identity;
                $user = $DB->get_record('user', [$syncconfig->users_primaryidentity => $identity]);
                if (!$user) {
                    continue;
                }
        ...
        if ($this->execute & SYNC_USER_SUSPEND) {

            $this->report(get_string('startingsuspendingusers', 'tool_sync'));
            if (!$adminuser = get_admin()) {
                return;
            }
            ...
            if (empty($this->manualfilerec)) {
                $filerec = $this->get_input_file(@$syncconfig->users_suspendfilelocation, 'usersuspend.csv');
            } else {
                $filerec = $this->manualfilerec;
            }
            ...
            $bom = "\xef\xbb\xbf";
            if (strpos($text, $bom) === 0) {
                // Do not bother anymore with BOM. Just remove it.
                // print_error("BOM detector : BOM is there ! It should have been removed from the incoming file.");
            ...
            while (!feof($filereader)) {
                ...
                $DB->set_field('user', 'suspended', 1, [$syncconfig->users_primaryidentity => $identity]);
                $userssuspended++;
            }

        }

        if ($this->execute & SYNC_USER_RESTORE) {

            $this->report(get_string('startingrestoringusers', 'tool_sync'));
