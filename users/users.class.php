...
class users_plugin_manager {
	...
	function config_form($frm) {
	    ...
	    include ($CFG->dirroot.'/admin/tool/sync/users/config.html');    
	}
	...
	*/
    function cron() {
        ...
        if (!file_exists($filename)) {
			tool_sync_report($CFG->tool_sync_userlog, get_string('filenotfound', 'tool_sync', $filename));
			return;        	
        }
			...
			if ($record[$header[0]]) {
				...
				foreach ($record as $name => $value) {
					...
					if (isset($required[$name]) and !$value) {
						...
						tool_sync_report($CFG->tool_sync_userlog, $errormessage);
						return;
				...
				if ($allowrenames && !empty($user->oldusername) ) {
					...
					if ($olduser = $DB->get_record('user', array('username' => $user->oldusername, 'mnethostid' => $user->mnethostid))) {
						if ($DB->set_field('user', 'username', $user->username, array('username' => $user->oldusername))) {
							tool_sync_report($CFG->tool_sync_userlog, get_string('userrenamed', 'admin')." : $user->oldusername $user->username");
							$renames++;
						} else {
							tool_sync_report($CFG->tool_sync_userlog, get_string('usernotrenamedexists', 'tool_sync')." : $user->oldusername $user->username");
							$renameerrors++;
							continue;
						}
					} else {
						tool_sync_report($CFG->tool_sync_userlog, get_string('usernotrenamedmissing', 'tool_sync')." : $user->oldusername $user->username");
						$renameerrors++;
						continue;
					}
				}
				...
				if ($olduser) {
					if ($updateaccounts) {
						...
						$user->id = $olduser->id;
						if ($olduser->deleted){
							tool_sync_report($CFG->tool_sync_userlog, get_string('userrevived', 'tool_sync', "$user->username ($idnumber)"));
						...
						tool_sync_report($CFG->tool_sync_userlog, "$olduser->id ".get_string('usernotaddedregistered', 'error', "[$username] $lastname $firstname ($user->idnumber)"));
						$userserrors++;
					}
				} else { // new user
					...
					foreach($addcourses as $c){
						...
						if (empty($c->wwwroot)){
							...
							if (!$crec = $DB->get_record('course', array('idnumber' => $c->idnumber))){		
								tool_sync_report($CFG->tool_sync_userlog, get_string('unknowncourse', 'error', $c->idnumber));
								continue;
							}
							...
							if (!empty($c->role)) {
								...
								$e->rolename = $c->role;
								tool_sync_report($CFG->tool_sync_userlog, get_string('roleadded', 'tool_sync', $e));
							} else {
								$ret = enrol_student($user->id, $crec->id);
							}
							if (@$ret) {   // OK
	                			tool_sync_report($CFG->tool_sync_userlog, get_string('enrolledincourse', '', $c->idnumber));
							} else {
	                			tool_sync_report($CFG->tool_sync_userlog, get_string('enrolledincoursenot', '', $c->idnumber));
							}
								...
								if (!empty($c->group)) {
									if (!$gid = groups_get_group_by_name($crec->id, $c->group)) {
										...
										if (!$gid = groups_create_group($groupsettings)) {
											tool_sync_report($CFG->tool_sync_userlog, get_string('groupnotaddederror', 'tool_sync', $c->group));
										}
									}
									...
									if ($gid){
										if (count(get_user_roles($coursecontext, $user->id))) {
											if (add_user_to_group($gid, $user->id)) {
					                			tool_sync_report($CFG->tool_sync_userlog, get_string('addedtogroup', '',$c->group));
											} else {
					                			tool_sync_report($CFG->tool_sync_userlog, get_string('addedtogroupnot', '',$c->group));
											}
										} else {
				                			tool_sync_report($CFG->tool_sync_userlog, get_string('addedtogroupnotenrolled', '', $c->group));
										}
									}
								}
							}
						...
						if (!empty($c->wwwroot) && $DB->get_record('block', array('name' => 'vmoodle'))){
							...
							$MNET->init();						
			                tool_sync_report($CFG->tool_sync_userlog, get_string('propagating', 'vmoodle', fullname($user)));
			                ...
			                if ($return = mnetadmin_rpc_user_exists($caller, $user->username, $c->wwwroot, true)){
			                	...
			                	if ($response->status == RPC_FAILURE_DATA){
			                		tool_sync_report($CFG->tool_sync_userlog, get_string('errorrpcparams', 'tool_sync', implode("\n", $response->errors)));
			                		continue;
			                	} elseif ($response->status == RPC_FAILURE){
			                		tool_sync_report($CFG->tool_sync_userlog, get_string('rpcmajorerror', 'tool_sync'));
			                		continue;
			                	} elseif ($response->status == RPC_SUCCESS){
			                		if (!$response->user){
				                		tool_sync_report($CFG->tool_sync_userlog, get_string('userunknownremotely', 'tool_sync', fullname($user)));
				                		$exists = false;
				                	} else {
				                		tool_sync_report($CFG->tool_sync_userlog, get_string('userexistsremotely', 'tool_sync', fullname($user)));
				                		$exists = true;
				                	}
			                	}
			                }
							...
							if (!$exists){
				                if ($return = mnetadmin_rpc_create_user($caller, $user->username, $user, '', $c->wwwroot, false)){
				                	...
				                	if (empty($response)){
			                			...
			                			tool_sync_report($CFG->tool_sync_userlog, get_string('remoteserviceerror', 'tool_sync'));
			                			continue;
				                	}
				                	if ($response->status != RPC_SUCCESS){
				                		// print_object($response);
			                			tool_sync_report($CFG->tool_sync_userlog, get_string('communicationerror', 'tool_sync'));
				                	} else {
				                		$u->username = $user->username;
				                		$u->wwwroot = $c->wwwroot;
			                			tool_sync_report($CFG->tool_sync_userlog, get_string('usercreatedremotely', 'tool_sync', $u));
				                		$created = true;
				                	}
				                }
				            }
				            ...
				            if (!empty($c->role)){
			                	...
			                	if ($response->status != RPC_SUCCESS){
			                		// print_object($response);
		                			tool_sync_report($CFG->tool_sync_userlog, get_string('communicationerror', 'tool_sync', implode("\n", $response->errors)));
			                	} else {			                		
			                		...
			                		if (file_exists($CFG->dirroot.'/blocks/user_mnet_hosts/xlib.php')){
			                			...
			                			if ($result = user_mnet_host_add_access($user, $c->wwwroot)){
			                				if (preg_match('/error/', $result)){
			                					tool_sync_report($CFG->tool_sync_userlog, get_string('errorsettingremoteaccess', 'tool_sync', $result));
			                				} else {
			                					tool_sync_report($CFG->tool_sync_userlog, $result);
			                				}
			                			}
			                		}
			                		...
			                		$e->wwwroot = $c->wwwroot;
		                			tool_sync_report($CFG->tool_sync_userlog, get_string('remoteenrolled', 'tool_sync', $e));
			                	}
				            }
						}
					}
